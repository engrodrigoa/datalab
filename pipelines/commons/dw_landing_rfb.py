...
#             logger.info(f"  Ensuring partition exists and truncating for {ref_mes_int}...")
#             nome_particao = f"{tabela}_{ref_mes_int}"
            
#             # 1. Cria a tabela partição para o mês específico, se ela não existir
#             cur.execute(f"""
#                 CREATE TABLE IF NOT EXISTS {schema}.{nome_particao} 
#                 PARTITION OF {schema}.{tabela} 
#                 FOR VALUES IN ({ref_mes_int})
#             """)
            
#             # 2. Faz o Truncate apenas na partição do mês (rápido e sem gerar WAL excessivo)
#             cur.execute(f"TRUNCATE TABLE {schema}.{nome_particao}")
            
#             logger.info(f"Partition {nome_particao} successfully truncated.")
