...
def processar_um_arquivo_com_retry(s3_client, chave_s3, chave_destino, max_tentativas=2):
    ...
    for tentativa in range(1, max_tentativas + 1):
        try:
            logger.info(f"Processing dimension socios in batched mode ({nome_arq}, batch size {TAMANHO_LOTE}, attempt {tentativa}/{max_tentativas})...")
            ...
            logger.info(f"Successfully processed and uploaded dimension socios to s3://{BUCKET_SILVER}/{chave_destino}")
...
def processar_socios_seguro(s3_client):
    ...
    if not arquivos:
        logger.warning(f"No Socios files found in s3://{BUCKET_BRONZE}/{prefixo_bronze}")
        return 0

    logger.info(f"Found {len(arquivos)} Socios files to process.")
    ...
    logger.info("==================================================================")
    logger.info(f"PIPELINE EXECUTION COMPLETED: {total_arquivos_processados} Socios files processed, {skipped_count} skipped (already in Silver layer).")
    logger.info("==================================================================")
