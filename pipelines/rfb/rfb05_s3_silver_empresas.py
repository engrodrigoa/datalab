...
def processar_um_arquivo_com_retry(s3_client, chave_s3, chave_destino, max_tentativas=2):
    ...
    for tentativa in range(1, max_tentativas + 1):
        try:
            logger.info(f"Processing dimension empresas in batched mode ({nome_arq}, batch size {TAMANHO_LOTE}, attempt {tentativa}/{max_tentativas})...")
            ...
            logger.info(f"Successfully processed and uploaded dimension empresas to s3://{BUCKET_SILVER}/{chave_destino}")
...
def processar_empresas_seguro(s3_client):
    logger.info("Starting Empresas pipeline (Bronze -> Silver) in batched mode.")
    ...
    arquivos = sorted([
        ...
        if "Empresas" in obj["Key"] and obj["Key"].endswith(".parquet")
    ])

    if not arquivos:
        logger.warning(f"No Empresas files found in s3://{BUCKET_BRONZE}/{prefixo_bronze}")
        return 0

    logger.info(f"Found {len(arquivos)} Empresas files to process.")
    ...
    for chave_s3 in arquivos:
        ...
        if not FORCAR_REPROCESSAMENTO and arquivo_ja_existe_no_silver(s3_client, chave_destino):
            logger.info(f"Skipping empresas ({nome_arq}): already exists in s3://{BUCKET_SILVER}/{chave_destino}")
            skipped_count += 1
            continue
    ...
    logger.info("==================================================================")
    logger.info(f"PIPELINE EXECUTION COMPLETED: {total_arquivos_processados} Empresas files processed, {skipped_count} skipped (already in Silver layer).")
    logger.info("==================================================================")
